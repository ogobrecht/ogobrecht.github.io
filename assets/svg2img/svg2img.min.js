...
!function(a){a.fn.svg2img=function(b){var c=a.extend({},a.fn.svg2img.defaults,b),d=a("body"),e=!1,f=[],g={};g.start=(new Date).getTime();var h=function(a){c.debug&&console.log("svg2img: "+a)},i=function(a){console.warn("svg2img: "+a)},j=function(b){var c=[],d=(b.ownerDocument||document).styleSheets;return a(d).each(function(d,e){a(e.cssRules).each(function(d,e){var f=!1;try{f=a(b).find(e.selectorText).length>0}catch(a){i("Unable to check if CSS selector is used: "+e.selectorText)}f&&c.push(e.cssText)})}),c},k=function(){var a=new Date,b=function(a){var b=Math.abs(Math.floor(a));return(b<10?"0":"")+b};return a.getFullYear()+b(a.getMonth()+1)+b(a.getDate())+"-"+b(a.getHours())+b(a.getMinutes())+b(a.getSeconds())},l=function(b,c){a(c.split(",")).each(function(a,c){["svg","png","gif","jpg","jpeg","bmp"].indexOf(c)<0?i('invalid image format "'+c+'" - possible formats are svg, png, gif, jpeg, bmp'):m(b,c)})},m=function(b,l){if(e)f.push({element:b,format:l}),c.debug&&h("export already running - queue additional "+l+" export");else{var n,o,p,q,r,s,t,u,v,w,x;e=!0,c.debug&&(g.startExport=(new Date).getTime(),h("1 - start "+("svg"!==l?l+" (canvas)":l)+" export")),d.append('<div id="tempContainerForExport" style="display:none;"></div>'),p=a("#tempContainerForExport"),n=a(b),n.clone(!1).appendTo(p),c.debug&&(g.endClone=(new Date).getTime(),h("2 - clone of original SVG done ("+(g.endClone-g.startExport)+"ms)")),o=p.find("svg"),o.prepend('<style type="text/css">'+j(n.parent()[0]).join(" ")+"</style>").attr("xmlns","http://www.w3.org/2000/svg").attr("version","1.1").attr("xmlns:xlink","http://www.w3.org/1999/xlink").css("border","none"),s=(n.attr("id")||n.parent().attr("id")||"export")+"-"+k()+"."+l,q=p.html().replace(/xmlns="http:\/\/www.w3.org\/2000\/svg"/g,'xmlns="http://www.w3.org/2000/svg"').replace(/xmlns:NS1=""/g,"").replace(/NS1:xmlns:xlink="http:\/\/www.w3.org\/1999\/xlink"/g,'xmlns:xlink="http://www.w3.org/1999/xlink"'),c.debug&&(g.endStylesAttributes=(new Date).getTime(),h("3 - collect styles and modify attributes done ("+(g.endStylesAttributes-g.endClone)+"ms)")),"svg"===l?(r=new Blob([q],{type:"image/svg+xml;charset=utf-8"}),saveAs(r,s),p.remove(),e=!1,c.debug&&(g.endSvgExport=(new Date).getTime(),h("4 - save to blob done ("+(g.endSvgExport-g.endStylesAttributes)+"ms)"),h("5 - runtime for file "+s+": "+(g.endSvgExport-g.startExport)+"ms")),f.length>0&&(x=f.shift(),m(x.element,x.format))):["png","gif","jpg","jpeg","bmp"].indexOf(l)>-1&&(d.append('<canvas id="tempCanvasForExport" style="display:none;"></canvas>'),t=a("#tempCanvasForExport").attr("height",n.attr("height")||parseFloat(n.css("height"))).attr("width",n.attr("width")||parseFloat(n.css("width"))),u=t[0].getContext("2d"),c.debug&&(g.endCanvasInit=(new Date).getTime(),h("4 - canvas init done ("+(g.endCanvasInit-g.endStylesAttributes)+"ms)")),w="data:image/svg+xml;base64,"+window.btoa(q),c.debug&&(g.endCreateDataUri=(new Date).getTime(),h("5 - create data URI done ("+(g.endCreateDataUri-g.endCanvasInit)+"ms)")),v=new Image,v.src=w,"jpg"===l&&(l="jpeg",i("wrong mime type jpg - normalized to jpeg")),"jpeg"===l&&(c.jpegQuality<0||c.jpegQuality>1)&&(c.jpegQuality=1,i("invalid value for jpegQuality - must be between 0 and 1 (1=100%)")),v.onload=function(){c.debug&&(g.endLoadImage=(new Date).getTime(),h("6 - load image (data URI) done ("+(g.endLoadImage-g.endCreateDataUri)+"ms)")),u.drawImage(v,0,0),c.debug&&(g.endDrawImage=(new Date).getTime(),h("7 - draw image on canvas done ("+(g.endDrawImage-g.endLoadImage)+"ms)")),t[0].toBlob(function(a){saveAs(a,s)},"image/"+l,c.jpegQuality),p.remove(),t.remove(),e=!1,c.debug&&(g.endCanvasExport=(new Date).getTime(),h("8 - save to blob done ("+(g.endCanvasExport-g.endDrawImage)+"ms)"),h("9 - runtime for file "+s+": "+(g.endCanvasExport-g.startExport)+"ms")),f.length>0&&(x=f.shift(),m(x.element,x.format))})}};return this.each(function(b,d){"svg"===d.tagName?l(d,c.format):a(d).find("svg").each(function(a,b){l(b,c.format)})}),this},a.fn.svg2img.defaults={format:"svg",jpegQuality:1,debug:!1}}(jQuery),/*! @source http://purl.eligrey.com/github/Blob.js/blob/master/Blob.js */
